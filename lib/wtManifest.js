...
wtManifest.prototype.computeMagnet = function (file, cn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    createTorrent(file, { announceList: this.announceList }, (err, t) => {
      ...
      // save .torrent file to use it as webseed 
      // add &ws=https://www.ffstream.com/chunks/file.ts&xs=https://www.ffstream.com/torrents/file.ts.torrent
      // to the end of magnet
      fs.writeFile(self.chunksLoc+'torrents/'+cn+'.torrent', t);
      const websedd = '&ws='+self.chunksLoc+cn+'&xs='+self.chunksLoc+'torrents/'+cn+'.torrent'
      const magnet = parseTorrent.toMagnetURI(parseTorrent(t))+websedd
      resolve(magnet)
    })
